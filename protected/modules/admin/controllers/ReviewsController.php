...
class ReviewsController extends AdminController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Reviews']))
		{
			...
			$model->img = $this->saveImage($model, 'img');
	...
	private function saveImage($model, $attr = 'img'){

		$old = $model->{$attr};
		
		$image = CUploadedFile::getInstance($model, $attr);

		//print_r($model->{$attr}); die();

		if($image){
			if(isset($model->id)) $model->removeImages();

			//$image = $model->{$attr};
			$upload_dir = Reviews::getUploadPath();

			//save image
			$filename = md5($image->getName().time()).'.'.$image->getExtensionName();
			
			Yii::app()->phpThumb->create($image->getTempName())
				->adaptiveResize(95, 95)->save($upload_dir.DIRECTORY_SEPARATOR.'mini'.$filename);
			return $filename;
		}else{
			return $old;
		}
